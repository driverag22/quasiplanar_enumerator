...
const std::size_t CYCLE_SIZE = 12;
...
int main() {
    ...
    while (solcount < solutions.size()) {
        ...
        for (int constrained = 0; constrained < 2; ++constrained) {
            ...
            std::size_t nm12 = d.vertices.size();
            d.add_vertices(CYCLE_SIZE);
            std::vector<Edge> local_edges = {
                {0,nm12+9}, // one matching
                // outer cycle
                {nm12+9 ,nm12+10,3},
                {nm12+10,nm12+11,3},
                {nm12+11,nm12   ,3},
                ...
                {nm12+8 ,nm12+9 ,3},
                // rest of matching
                // EVEN: (i, nm12 + ( (i+9) % 12)) == (i, nm12 + ( (i-3) % 12))
                // ODD: (i, nm12 + ( (i+3) % 12)) == (i, nm12 + ( (i-9) % 12))
                {2,nm12+11},{4,nm12+1},{6,nm12+3},{8,nm12+5},{10,nm12+7},{12,nm12+9},
                {1,nm12+4},{3,nm12+6},{5,nm12+8},{7,nm12+10},{9,nm12+11},{11,nm12+2},{13,nm12+4}
            };
...
BACKUP:
                ...
                if (++e == local_edges.end()) {
                    if (constrained == 0) {
                        ...
                        std::size_t active_cycle_end = nm12 + CYCLE_SIZE;
                        for (auto i = d.halfedges.begin(); i != d.halfedges.end(); ++i) {
                            // active cycle vertex -> incident face
                            if (i->vertex->label >= nm12 && i->vertex->label < active_cycle_end)
                                net.add_edge(num_faces + (i->vertex->label - nm12), face[i->label], 1);
                        ...
                        for (std::size_t i = 0; i < num_faces; ++i) {
                            ...
                            do {
                                ...
                                if (v >= nm12 && v < active_cycle_end) // active vertex
                        ...
                        for (auto i = d.vertices.begin(); i != d.vertices.end(); ++i) {
                            ...
                            if (i->label >= nm12 && i->label < active_cycle_end) {
                        ...
                        std::string gml_path = "../quasiDrawings/nested_c12/solution_" + std::to_string(fullsolcount) + ".graphml";
                        std::ofstream of_gml(gml_path);
                        d.graphml_output(of_gml);
                        of_gml.close();

                        std::string json_path = "../quasiDrawings/nested_c12/solution_" + std::to_string(fullsolcount) + ".json";
