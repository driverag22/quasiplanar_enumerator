...
int main() {
    ...
    for (int i = 0; i < 1607; i++) {
        ...
        for (const auto& item : all_edge_sets) {
            std::cout << "Permutation [" << idx << " / " << total_edge_sets << "]\n\n";
            const Edges& current_edges = item.edges;
            const auto& missing_edges = item.missing_edges;

            auto start_edge = current_edges.begin();
            const size_t total_edges = current_edges.size();
            std::vector<int> path_choice(current_edges.size(), 0); 
            ...
            for (auto e = start_edge;;) {
                ...
                if (total_iterations % 100000 == 0) {
                    std::cout << 
                        "\r\033[K Iter: " << total_iterations << " | Depth: " << current_depth << "/" << total_edges << " | Max Depth: " << max_depth_reached << " | Choices: [";
